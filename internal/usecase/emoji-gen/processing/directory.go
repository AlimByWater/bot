...
func (m *Module) CheckAndRemoveOldDirectories() {
	...
	m.directories.Range(func(key, value interface{}) bool {
		...
		if err != nil {
			m.logger.Debug("Failed to stat directory", slog.String("dir", dir), slog.String("err", err.Error()))
			return true
		}

		remove := info.ModTime().Add(entity.DirectoryLifeTime).Before(now)
		if remove {
			m.logger.Info("Removing old directory", slog.String("dir", dir))
			if err := os.RemoveAll(dir); err != nil {
				m.logger.Debug("Failed to remove directory", slog.String("dir", dir), slog.String("err", err.Error()))
				return true
			}
		}
