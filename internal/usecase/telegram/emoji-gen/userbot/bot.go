...
func (u *User) Init(ctx context.Context, logger *slog.Logger) error {
	u.ctx = ctx
	logger = logger.With(slog.String("module", "USERBOT"))
	...
	dispatcher.AddHandlerToGroup(handlers.NewMessage(filters.Message.Text, u.saveAccessHash), 0) // сохраняем хэш доступа в базу данных
...
func (u *User) echo(ctx *ext.Context, update *ext.Update) error {
	//slog.Info("input peer", update.EffectiveChat().GetInputChannel(), update.EffectiveChat().GetInputPeer(), update.UpdateClass.TypeName())
	...
	default:
		...
		slog.Info("input peer", slog.String("peer", peer.String()))
...
func (u *User) saveAccessHash(ctx *ext.Context, update *ext.Update) error {
	if !update.GetUserChat().Bot {
		return nil
	}

	chatID := fmt.Sprintf("-100%d", update.EffectiveChat().GetID())

	accessHash := entity.AccessHash{
		Username: update.GetUserChat().Username,
		ChatID:   chatID,
		Hash:     update.EffectiveChat().GetAccessHash(),
		PeerID:   update.EffectiveChat().GetID(),
	}
