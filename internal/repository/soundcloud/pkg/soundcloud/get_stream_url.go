...
func (s *Soundcloud) ConstructStreamURL(trackUrl string, doc *html.Node) (string, error) {

	// get client id
	clientID, err := s.GetClientID(trackUrl)
	if err != nil {
		return "", err
	}
	...
	baseURL := "https://api-v2.soundcloud.com/media/soundcloud:tracks:%s/%s/stream/hls?client_id=%s"

	streamURL := fmt.Sprintf(baseURL, trackID, streamToken, clientID)
